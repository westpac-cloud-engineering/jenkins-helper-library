...
def call(body) {
    ...
    stage ('Setting Up Deployment File') {
        ...
        def deployment_map = withCredentials(
            ...
            deployment_map['deployment'] = ['id': config.app_id, 'component_name': config.app_component_name, 'environment': config.app_environment, 'destroy': config.destroy_flag, 'branch_or_tag': config.branch_or_tag ]
            deployment_map['consul'] = ['address': "consul.australiaeast.cloudapp.azure.com", 'port': 8500, 'dc':"australiaeast","token":""]
            deployment_map['service_now'] = ['url': "https://wbchpaaspoc.service-now.com/api/now/table/change_request", 'username': env.JENKINS_SNOW_USERNAME, 'password':env.JENKINS_SNOW_PASSWORD]
